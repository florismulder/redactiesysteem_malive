...
// ─── kleuren ──────────────────────────────────────────────
const roleColors = {
  Eindredactie: "#FF6B35", Host: "#4ECDC4",
  Techniek: "#FFE66D", Nieuwsredactie: "#A8E6CF",
};
const typeConfig = {
...
function EF({ label, value, onChange, multiline=false, placeholder="" }) {
  const s={width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",
    color:"#ccc",padding:"5px 8px",fontSize:12,fontFamily:"'IBM Plex Mono',monospace",
    borderRadius:3,boxSizing:"border-box",resize:"vertical"};
  return (
    <div style={{marginBottom:5}}>
      {label&&<div style={{fontSize:9,letterSpacing:2,color:"#555",textTransform:"uppercase",marginBottom:2}}>{label}</div>}
      {multiline
...
function ItemCard({ item, role, onUpdate, onDuurChange, onZoek, isActive, isPast }) {
  ...
  return (
    <div style={{display:"flex",opacity:dimmed?0.28:1,transition:"opacity 0.4s",
      ...
      <div style={{width:58,flexShrink:0,paddingTop:10,textAlign:"right",paddingRight:12}}>
        <div style={{fontSize:11,fontFamily:"'IBM Plex Mono',monospace",fontWeight:isActive?700:400,
      ...
      <div style={{flex:1,background:"rgba(255,255,255,0.02)",borderRadius:"0 4px 4px 0",padding:"7px 12px",margin:"4px 0"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:canEdit?5:0}}>
          <span style={{fontSize:9,letterSpacing:2,color:tc.color,fontWeight:700}}>{tc.icon} {tc.label}</span>
          <span style={{fontSize:12,color:"#ddd",fontWeight:600}}>{item.what}</span>
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono','Courier New',monospace",background:"#0D0D0D",minHeight:"100vh",color:"#bbb"}}>
      <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;700&family=IBM+Plex+Sans:wght@300;600;700&display=swap" rel="stylesheet"/>

      {/* ── Header ── */}
      <div style={{background:"#111",borderBottom:"1px solid #1e1e1e",padding:"0 20px",display:"flex",alignItems:"center",height:52,gap:16}}>
        <div style={{display:"flex",alignItems:"baseline",gap:5}}>
          <span style={{fontSize:17,fontWeight:700,color:"#FF6B35",letterSpacing:-1,fontFamily:"'IBM Plex Sans',sans-serif"}}>MA</span>
          <span style={{fontSize:17,fontWeight:300,color:"#fff",fontFamily:"'IBM Plex Sans',sans-serif"}}>LIVE</span>
          <span style={{fontSize:9,color:"#333",letterSpacing:3,marginLeft:6}}>DRAAIBOEK</span>
        </div>
        <div style={{flex:1}}/>
        <SyncBadge status={syncStatus}/>
        <div style={{fontSize:10,color:"#3a3a3a"}}>Do 19 feb 2026</div>
        <div style={{fontSize:13,color:"#666",fontWeight:700,fontFamily:"'IBM Plex Mono',monospace"}}>
          {String(now.getHours()).padStart(2,"0")}:{String(now.getMinutes()).padStart(2,"0")}:{String(now.getSeconds()).padStart(2,"0")}
        </div>
      </div>

      {/* ── Rolbalk ── */}
      <div style={{background:"#121212",borderBottom:"1px solid #1a1a1a",padding:"7px 20px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        <span style={{fontSize:9,letterSpacing:2,color:"#3a3a3a"}}>ROL</span>
        {["Eindredactie","Host","Techniek","Nieuwsredactie"].map(r=>(
          <button key={r} onClick={()=>setRole(r)} style={{padding:"3px 12px",borderRadius:20,border:"1px solid",
            ...
            color:role===r?roleColors[r]:"#484848"}}>{r}</button>
        ))}
        <div style={{flex:1}}/>
      ...
      <div style={{display:"flex",height:"calc(100vh - 95px)",overflow:"hidden"}}>
        {/* ── Sidebar ── */}
        <div style={{width:172,background:"#0f0f0f",borderRight:"1px solid #181818",flexShrink:0,overflowY:"auto"}}>
          <div style={{paddingTop:12}}>
            {[{id:1,l:"UUR 1",s:"12:00 – 13:00"},{id:2,l:"UUR 2",s:"13:00 – 14:00"},{id:3,l:"GASTEN",s:""},{id:4,l:"REDACTIE",s:""}].map(t=>(
              <button key={t.id} onClick={()=>setTab(t.id)} style={{width:"100%",textAlign:"left",padding:"9px 16px",
                background:tab===t.id?"#181818":"transparent",border:"none",
                borderLeft:`3px solid ${tab===t.id?"#FF6B35":"transparent"}`,
                color:tab===t.id?"#eee":"#484848",cursor:"pointer",fontSize:11}}>
          ...
          <div style={{padding:"14px 16px 0",marginTop:8,borderTop:"1px solid #181818"}}>
            <div style={{fontSize:8,letterSpacing:2,color:"#2a2a2a",marginBottom:8}}>LEGENDA</div>
            ...
            <div style={{marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                ...
                <span style={{fontSize:9,color:"#FF6B35"}}>
                  {curSec>=(tab===1?43200:46800)&&curSec<(tab===1?46800:50400)?`▶ LIVE ${curStr}`:""}
                </span>
              </div>
              <div style={{height:2,background:"#181818",borderRadius:2}}>
                <div style={{height:"100%",borderRadius:2,background:"#FF6B35",transition:"width 1s linear",width:`${pct(tab)}%`}}/>
              </div>
            </div>
...
function GastenTab({ uitzendingId, setSyncStatus }) {
  ...
  return (
    <div>
      <div style={{fontSize:9,letterSpacing:3,color:"#3a3a3a",marginBottom:14}}>GASTENLIJST</div>
      {gasten.map((g,i)=>(
        <div key={g.id} style={{background:"#111",border:"1px solid #1e1e1e",borderRadius:4,padding:13,marginBottom:8}}>
          <div style={{fontSize:9,color:"#FF6B35",marginBottom:8,letterSpacing:2}}>GAST {i+1}</div>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:8,marginBottom:6}}>
            <EF label="Naam" value={g.wie} onChange={v=>upd(g.id,"wie",v)} placeholder="Naam gast"/>
            <EF label="Studio / Tel" value={g.type} onChange={v=>upd(g.id,"type",v)} placeholder="Studio"/>
            <EF label="Telefoonnummer" value={g.tel} onChange={v=>upd(g.id,"tel",v)} placeholder="06-…"/>
          </div>
          <EF label="Onderwerp" value={g.onderwerp} onChange={v=>upd(g.id,"onderwerp",v)} placeholder="Gespreksonderwerp"/>
        </div>
      ))}
      <button onClick={()=>setGasten(p=>[...p,{id:Date.now(),wie:"",type:"Telefonisch",tel:"",onderwerp:""}])}
        style={{padding:"7px 14px",background:"transparent",border:"1px dashed #252525",
          color:"#3a3a3a",cursor:"pointer",fontSize:10,borderRadius:3}}>+ Gast toevoegen</button>
    </div>
  );
}
